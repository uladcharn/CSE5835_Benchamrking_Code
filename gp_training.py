...
N = len(ds_grouped)
print(f"Total number of data per set: {N}")
...
#     this for loop ends when all candidates in pool are observed 
    for i in np.arange(len(index_learn)):
        ...
        y_train = y_scaler.fit_transform(np.array(y_).reshape(-1,1))
...
#             #TODO: select kernel for GP surrogate model
            ...
            GP_learn.Gaussian_noise.variance = 1e-3
            GP_learn.Gaussian_noise.variance.constrain_bounded(1e-6, 1e-1)  
            GP_learn.kern.Mat52.variance.constrain_bounded(1e-3, 1e3)
            GP_learn.kern.Mat52.lengthscale.constrain_bounded(1e-3, 1e3)
...
#         we choose candidate with larger acquisition function value to be observed next   
        ...
        for j in index_learn:
            X_j = X_feature[j]
            X_j_scaled = s_scaler.transform([X_j])
            y_j = y[j]
#             #TODO: select Acquisiton Function for BO
            ac_value = LCB(X_j_scaled, GP_learn, 2)
...
print('Saving model to a .npy object...')
total_time = time.time() - start_time

master = np.array([index_collection, X_collection, y_collection, TopCount_collection, total_time], dtype=object)
np.save(f'test_run_gp_{dataset_name}_{seed_num}', master)
