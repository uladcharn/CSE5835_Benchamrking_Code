...
This work represents the final project for CSE 5835 (Machine Learning for Physical Sciences) class at University of Connecticut.
...
Original GitHub Repo: https://github.com/PV-Lab/Benchmarking
...
run `rf_training.py` or `gp_training.py` with any of the given datasets to benchmark the performance of a selected BO algorithm using pool-based active learning framework. 
run `performance_visualization.py` with given demo benchmarking results or any benchmarking results from running the framework locally.

The package contains the following module and scripts:

| Module | Description |
| ------------- | ------------------------------ |
| `rf_training.py`      | Script for running Random Forest framework      |
| `gp_training.py`      | Script for running gaussian Process with Automatic Relevance Detection framework       |
| `performance_visualization.py`      | Script for visualizing materials dataset design spaces   |
