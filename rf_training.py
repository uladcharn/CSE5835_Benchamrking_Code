...
#     this for loop ends when all candidates in pool are observed 
    candidate_indices = list(index_learn)
    X_candidates_raw = X_feature[candidate_indices]
...
master = np.array([index_collection, X_collection, y_collection, TopCount_collection], dtype=object)
np.save(f'test_run_rf_{dataset_name}_{seed_num}', master)
